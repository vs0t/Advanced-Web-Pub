var InsertCourse = React.createClass({
  ...
  validateInput: function(coursePrefix, courseNumber, section) {
      ...
      if (!/^[A-Za-z]+$/.test(coursePrefix) || coursePrefix.length >= 5 || coursePrefix.length <= 1) {
      ...
      if (!/^\d+$/.test(courseNumber) || courseNumber.length <= 1 || courseNumber.length >= 5) {
  ...
  handleSubmit: function(event) {
      ...
      var { faculty, semester, year, coursePrefix, courseNumber, section } = this.state;
      ...
      // Validate input
      var validationError = this.validateInput(coursePrefix, courseNumber, section);
      if (validationError) {
      ...
      var courseData = JSON.stringify({ faculty, semester, year, coursePrefix, courseNumber, section });
                  ...
                  <form onSubmit={this.handleSubmit}>
                      <table>
                          <tbody>
                              ...
                              <tr>
                                  <td><label htmlFor="coursePrefix">Course Prefix:</label></td>
                                  <td><input type="text" name="coursePrefix" value={this.state.coursePrefix} onChange={this.handleChange} /></td>
                              </tr>
                              <tr>
                                  <td><label htmlFor="courseNumber">Course Number:</label></td>
                                  <td><input type="text" name="courseNumber" value={this.state.courseNumber} onChange={this.handleChange} /></td>
                              </tr>
                              <tr>
                                  <td><label htmlFor="section">Section:</label></td>
                                  <td><input type="text" name="section" value={this.state.section} onChange={this.handleChange} /></td>
                              </tr>
                              <tr>
                                  <td colSpan="2">
                      ...
                      {this.state.validationError && <p style={{ color: 'red' }}>{this.state.validationError}</p>}
                  </form>
              </div>
          </div>
      );
