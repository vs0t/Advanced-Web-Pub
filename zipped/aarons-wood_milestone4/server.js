...
app.get("/searchplayers", function (req, res) {
  ...
  // Construct the SQL query
  let sqlEA = `SELECT * FROM Player WHERE `;
  let conditionsEA = [];

  // Add conditions for each filter, if provided
  if (firstNameEA) conditionsEA.push(`PlayerFirstName LIKE '%${firstNameEA}%'`);
  if (lastNameEA) conditionsEA.push(`PlayerLastName LIKE '%${lastNameEA}%'`);
  if (emailEA) conditionsEA.push(`PlayerEmail LIKE '%${emailEA}%'`);
  if (addressEA) conditionsEA.push(`PlayerAddress LIKE '%${addressEA}%'`);
  if (cityEA) conditionsEA.push(`PlayerCity LIKE '%${cityEA}%'`);
  if (stateEA) conditionsEA.push(`PlayerState LIKE '%${stateEA}%'`);
  if (zipEA) conditionsEA.push(`PlayerZip LIKE '%${zipEA}%'`);
...
app.get("/searchreservations", function (req, res) {
  ...
  if (reservationsDateEA) conditionsEA.push(`r.ReservationsDate = '${reservationsDateEA}'`);
  if (reservationsTimeEA) conditionsEA.push(`r.ReservationsTime = '${reservationsTimeEA}'`);
  if (reservationsCountEA) conditionsEA.push(`r.ReservationsCount = ${reservationsCountEA}`);
