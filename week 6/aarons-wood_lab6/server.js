...
app.post('/updatesinglecus', function (req, res) {
    ...
    var sqlins = "UPDATE customertable SET dbcustomername = ?, dbcustomeraddress = ?, dbcustomerzip = ?, dbcustomercredit = ?, dbcustomeremail = ?, dbcustparticipant = ?, dbcustreward = ? " +
    " WHERE dbcustomerid = ?";
    var inserts = [cname, caddress, czip, ccredit, cemail, cparticipant, creward, cid]
    var sql = mysql.format(sqlins, inserts);
    console.log(sql);
    con.execute(sql, function (err, result) {
        if (err) throw err;
